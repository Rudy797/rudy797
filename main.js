var mainVersion='0.784';var menuVis=false;var srchVis=false;function absolute(base,relative){var stack=base.split("/"),parts=relative.split("/");stack.pop();for(var i=0;i<parts.length;i++){if(parts[i]==".")
continue;if(parts[i]=="..")
stack.pop();else
stack.push(parts[i]);}
return stack.join("/");}
...
var adIDs=[['adTop',1,0],['adend',1,0],['hideAds1',1,0],['showAds1',0,1]];function sethideadscookie(val){var date=new Date();if(val==1)
...
function setAds(stateNo){for(var i=0;i<adIDs.length;i++){var div=document.getElementById(adIDs[i][0]);if(div){var styleNo=adIDs[i][stateNo+1];switch(styleNo){case 0:div.style.display='none';break;case 1:div.style.display='inline';break;}}}}
function hideAds(){if(confirm("This will set a Cookie so we can remember not to show ads on each page. It lasts for about a day. Do you agree?")){sethideadscookie(1);setAds(1);}else{sethideadscookie(0);}}
function showAds(){sethideadscookie(0);setAds(0);}
function cookieHideAds(){var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++){var c=ca[i];var pair=c.split('=');var key=pair[0];var value=pair[1];key=key.replace(/ /,'');if(key=='math_hideads'){console.log("cookieHideAds");setAds(1);}}}
...
s+='</ul>';s+='</li>';s+='</ul>';}
if(typ==2){s+='&nbsp;';s+='<a href="javascript:showMenu()" style="text-decoration: none;" aria-label="Show Menu">';s+='<img src="'+urlStt+'images/style/menu.svg" alt="Menu" />';s+='</a>';s+=' ';s+='<a href="javascript:showSrch()" style="text-decoration: none;" aria-label="Show Search">';s+='<img src="'+urlStt+'images/style/search.gif" alt="Search" />';s+='</a>';}
return s;}
...
function getSearch(){var s="";s+='<form action="../search/search.php" method="get">';s+='<input type="text" name="query" value="" placeholder="Search" id="searchFld" aria-label="Search" />';s+='<input type="submit" name="submit" value="" id="searchBtn" aria-label="Search Button" />';s+='<input type="hidden" name="search" value="1" />';s+='</form>';return s;}
...
function getAdTop(){var path=location.pathname;path=path.split('/mathsisfun/').join('/');var dir=path.substring(0,path.lastIndexOf('/'));var file=path.substring(path.lastIndexOf('/')+1);console.log('getAdTop',dir,file)
var s=''
...
return s;}
function getAdEnd(){var path=location.pathname;path=path.split('/mathsisfun/').join('/');var dir=path.substring(0,path.lastIndexOf('/'));var file=path.substring(path.lastIndexOf('/')+1);console.log('getAdEnd',dir,file)
var src=''
src='adsense'
var s=getAdCode(src)
...
function getFooter(){var s='';var sep=' &cir; '
s+='<div id="footMenu">';s+='<a href="../search/search.php">Search</a>'+sep+'<a href="../links/index.html">Index</a>'+sep+'<a href="../aboutmathsisfun.html">About</a>'+sep+'<a href="../contact.php">Contact</a>'+sep+'<a href="javascript:Citation()">Cite&nbsp;This&nbsp;Page</a>'+sep+'<b><a href="../Privacy.htm">Privacy</a></b>';s+='</div>';return s;}
function getBodyEnd(){cookieHideAds();var s="";s+=gAnalyticsCall();return s;}
...
function logVisit(){var pg=location.pathname;if(pg=='/')return;var pgHex=toHex(pg);addView(pgHex,"Visit",window.location.hostname);}
if(Math.random()<0.1)logVisit();function addView(pg,viewtype,hostname){var req;req=new XMLHttpRequest();var params="type="+viewtype;params+="&pg="+encodeURIComponent(pg);if(hostname=="localhost"){req.open("POST","http://localhost/mathopolis/links/update.php",true);}else{req.open("POST","https://www.mathsisfun.com/links/update.php",true);}
req.setRequestHeader("Content-type","application/x-www-form-urlencoded");req.send(params);console.log("addView");}
